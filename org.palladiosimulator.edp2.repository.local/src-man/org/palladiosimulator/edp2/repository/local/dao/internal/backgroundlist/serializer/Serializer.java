...
public interface Serializer<T> {
    ...
    /**
     * @return The size of each element in serialised form, e.g., for doubles it could be 8 bytes
     *         (IEEE format). The result is not allowed to change at runtime
     */
    long getElementLength();

    /**
     * Convert the given array of elements into their serialised form. Attention: Due to a flaw in
     * Java Generics, this signature cannot be serialise(T[] objects, int count) as this leads to a
     * class cast exception despite the fact that you pass the right array. We kept the Type
     * parameter to get at least a type save deserialise.
     * 
     * @param objects
     *            The array of elements to serialise
     * @param count
     *            The number of elements to serialise
