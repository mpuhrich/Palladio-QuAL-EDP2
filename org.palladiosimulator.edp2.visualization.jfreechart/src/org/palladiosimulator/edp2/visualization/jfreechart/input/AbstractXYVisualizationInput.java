...
public abstract class AbstractXYVisualizationInput extends JFreeChartVisualizationInput {
    ...
    protected void firstChildInputAdded(final JFreeChartVisualizationSingleDatastreamInput newChildInput) {
        ...
        final MetricSetDescription metricSetDescription = ((MetricSetDescription) newChildInput.getDataSource().getMetricDesciption());
        
        xPos = 0;
        if(metricSetDescription.getSubsumedMetrics().get(1).getId().equals(MetricDescriptionConstants.POINT_IN_TIME_METRIC.getId())) {
