 ...
 * <ul>
 *   <li>{@link org.palladiosimulator.edp2.models.Repository.impl.LocalDirectoryRepositoryImpl#getUri <em>Uri</em>}</li>
 * </ul>
 * </p>
 *
 * @generated
 */
public class LocalDirectoryRepositoryImpl extends RepositoryImpl implements
	...
	protected LocalDirectoryRepositoryImpl() {
		super();
		final MetaDaoDelegate delegate = this
				.getMetaDaoDelegate("org.palladiosimulator.edp2.dao.localfile.dao");
	...
	private MetaDaoDelegate getMetaDaoDelegate(final String wantedId) {
		try {
			return ExtensionHelper.getExecutableExtension(
					"org.palladiosimulator.edp2.dao", "class", "id", wantedId);
		} catch (final RuntimeException e) {
		}

		try {
			return (MetaDaoDelegate) Class
					.forName(
							"org.palladiosimulator.edp2.dao.localfile.LocalDirectoryMetaDao")
					.newInstance();
	...
	public void setUri(final String newUri) {
		...
		if (this.eNotificationRequired()) {
			this.eNotify(new ENotificationImpl(this, Notification.SET,
					RepositoryPackage.LOCAL_DIRECTORY_REPOSITORY__URI, oldUri,
					this.uri));
		}
	}
	 ...
	 */
	@Override
	public File convertUriStringToFile(final String uriString)
			throws DataNotAccessibleException {
	 ...
	 */
	private File convertUriStringToFileInternal(final String uriString)
			throws DataNotAccessibleException {
		...
		if (uri.isPlatform()) {
			...
			try {
				...
				throw new DataNotAccessibleException(
						"The URI is not well-formed.", e);
			} catch (final IOException e) {
				throw new DataNotAccessibleException(
						"The URI could not be converted.", e);
			}
		} else {
		...
		if (fileLocation == null) {
			...
			throw new DataNotAccessibleException(
					"The URI could not be converted to a local file.", null);
		} else {
			directory = new File(fileLocation);
			if (!directory.isDirectory()) {
				// URI does not point to a directory.
				throw new DataNotAccessibleException(
						"The URI does not point to a directory.", null);
			}
		}
		return directory;
	}
